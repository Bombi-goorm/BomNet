...
import com.bombi.core.common.dto.CommonResponseDto<;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> handleMethodArgumentNotValidException(MethodArgumentNotValidException ex) {
        ...
        if (errorMessages.size() == 1) {
            ...
            CommonResponseDto<> response = new CommonResponseDto<>("FAILURE", errorMessages.get(0));
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
        } else {
            // 오류 메시지가 여러개인 경우
            CommonResponseDto<> response = new CommonResponseDto<>("FAILURE", "요청이 유효하지 않습니다.", errorMessages);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
        }
    }
    ...
    public ResponseEntity<?> handleHttpMessageNotReadableException(HttpMessageNotReadableException ex) {
        ...
        CommonResponseDto<> response = new CommonResponseDto<>("FAILURE", "요청 본문이 비어있거나 올바르지 않습니다.");
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    }
    ...
    public ResponseEntity<?> handleDataIntegrityViolation(DataIntegrityViolationException ex) {
        ...
        CommonResponseDto<> response = new CommonResponseDto<>("FAILURE", "필수 데이터가 누락되었거나 잘못된 데이터가 입력되었습니다.");
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    }
    ...
    public ResponseEntity<?> handleNoResourceFoundException(NoResourceFoundException ex) {
        ...
        CommonResponseDto<> response = new CommonResponseDto<>("FAILURE", "잘못된 요청입니다.");
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    }
    @ExceptionHandler(HttpRequestMethodNotSupportedException.class)
    public ResponseEntity<?> handleHttpRequestMethodNotSupportedException(HttpRequestMethodNotSupportedException ex) {
        log.warn("HttpRequestMethodNotSupportedException - " + ex.getMessage());
        CommonResponseDto<> response = new CommonResponseDto<>("FAILURE", "지원하지 않는 요청입니다.");
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    }
    @ExceptionHandler(MaxUploadSizeExceededException.class)
    public ResponseEntity<?> handleMaxUploadSizeExceededException(MaxUploadSizeExceededException ex) {
        log.warn("MaxUploadSizeExceededException - " + ex.getMessage());
        CommonResponseDto<> response = new CommonResponseDto<>("FAILURE", "최대 업로드 용량을 초과했습니다. (최대 5MB).");
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    }
    ...
    public ResponseEntity<?> handleRedisSessionException(RedisSessionException ex) {
        ...
        CommonResponseDto<> response = new CommonResponseDto<>(ex.getStatus(), message);
    ...
    public ResponseEntity<?> handleNaverApiException(NaverApiException ex) {
        ...
        CommonResponseDto<> response = new CommonResponseDto<>(ex.getStatus(), "요청이 실패했습니다. 관리자에게 문의해주세요.");
    ...
    public ResponseEntity<?> handleTestbedApiException(TestbedApiException ex) {
        ...
        CommonResponseDto<> response = new CommonResponseDto<>(ex.getStatus(), ex.getMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    }
    ...
    public ResponseEntity<?> handleServerError(Exception ex) {
        ...
        // 사용자에게 반환할 메시지
        CommonResponseDto<> response = new CommonResponseDto<>("FAILURE", "서버 내부 오류가 발생했습니다. 관리자에게 문의해주세요.");
    ...
    public ResponseEntity<?> handleIllegalStateException(IllegalStateException ex) {
        ...
        CommonResponseDto<> response = new CommonResponseDto<>("FAILURE", "서버 내부 오류가 발생했습니다. 잠시 후 다시 시도해주세요.");
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
    }
}
