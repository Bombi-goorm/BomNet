...
public interface RegionWeatherRepository extends JpaRepository<RegionWeather, Long> {
	...
	@Query("select rw from RegionWeather rw join fetch rw.region r where r.kmaSidoCode = :sidoCode")
	Optional<RegionWeather> findWeatherInfoByPNU(@Param("sidoCode") String sidoCode);
}
